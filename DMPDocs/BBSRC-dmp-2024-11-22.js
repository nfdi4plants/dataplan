BBSRC_dmp = {"BBSRC-dmp" : `<div id="BBSRC-dmp" class="">
    <div class="content-page" id="text_start">
        <span class="c2">Data management plan of $_PROJECTNAME for BBSRC</span>
        <ul class="c0 ">

            <li class="c0 li-bullet-0"><span class="c1">Data Areas and Data Types <br> &ndash; list-dataCollection-en </span></li>

            <span class="c1"> text-data-size-en </span>
            <p class="c0"><span class="c1"></span></p>
            <li class="c0 li-bullet-0"><span class="c1">Standards and Metadata <br> &ndash;&nbsp;                
           text-standards-long-en

                </span>
            </li>
            <li class="c0 li-bullet-0"><span class="c1">Reuse of published data <br> &ndash; list-reuse-en</span></li>
            <li class="c0 li-bullet-0"><span class="c1">Secondary Use <br> &ndash; text-data-utility-en </span></li>

            <li class="c0 li-bullet-0"><span class="c1">Methods for Data Sharing  <p class="c0">
                        <span class="c1">
                        text-repository-long-en
                        text-dataplant-tools-en
                        </span>
                    </p></span></li> <li class="c0 li-bullet-0"><span class="c1">Proprietary Data <br> &ndash; Open public
                data will be used whenever possible. </span></li>
            <li class="c0 li-bullet-0"><span class="c1">Timeframes <br>
                    &ndash; text-made-public-en
                </span></li>
            <li class="c0 li-bullet-0"><span class="c1"> text-metadata-format-en </span></li>
    </div>
</div>`}